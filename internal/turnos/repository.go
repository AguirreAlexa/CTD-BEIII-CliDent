...
type Repository interface {
	...
	Save(ctx context.Context, t domain.Turno) (int, error)
...
func (r *repository) Save(ctx context.Context, t domain.Turno) (domain.Turno, error) {
...
func (r *repository) GetByPacienteDNI(ctx context.Context, dni int) ([]domain.Turno, error) {
	query := "SELECT t.id, t.fecha, t.hora, t.descripcion, d.id, d.nombre, d.apellido, d.matricula, p.id," + 
	" p.nombre, p.apellido, p.domicilio, p.dni, p.fecha_de_alta FROM turnos t JOIN dentistas d ON d.matricula = t.dentista_matricula" +
	" JOIN pacientes p ON p.dni = t.paciente_dni WHERE t.paciente_dni=?;"
